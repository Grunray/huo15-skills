...
**脚本做了什么**：
1. 动态调用聚星逸 API 获取最新模型列表
2. 自动分类（文本对话 vs 生图/视频）
3. 写入 `~/.openclaw/openclaw.json`
4. 默认设 `DeepSeek-V4-Flash` 为主模型
5. 自动备份原配置
...
⚡ Flash（快速）
  DeepSeek-V4-Flash               128K ctx  推理  ← 默认
  Gemini-3.5-Flash               1024K ctx
  GLM-5-Turbo                     128K ctx
  ...

🚀 Pro（主力）
  DeepSeek-V4-Pro                 128K ctx  推理
  GPT-5.5                         128K ctx  推理
  claude-opus-4-8                 195K ctx  推理
  ...

🧠 Reasoner（深度推理）
  DeepSeek-R1-0528                128K ctx  推理
  ...
```
...
🛰️  聚星逸当前配置
   ...
   已配模型:
   ★  DeepSeek-V4-Flash               128K   推理
      DeepSeek-V4-Pro                 128K   推理
      ...
```
...
| 场景 | 推荐模型 | Tier |
|------|---------|------|
| 日常编程、快速问答 | DeepSeek-V4-Flash | flash |
| 复杂代码生成、架构设计 | DeepSeek-V4-Pro 或 GPT-5.5 | pro |
| 深度分析、数学推理 | DeepSeek-R1-0528 | reasoner |
| 长文档处理（1M 上下文） | Gemini-3.5-Flash | flash |
| Claude 系列偏好 | claude-opus-4-8 | pro |
| 国产模型偏好 | Qwen3.7-Max 或 GLM-5.2 | pro |
...
# 选最近的
cp ~/.openclaw/openclaw.json.bak.2026-07-11T00-00-00-000Z ~/.openclaw/openclaw.json
openclaw restart
```
...
脚本会动态拉取最新列表，自动包含新模型。无需更新 skill 本身。
...
```bash
# ── 配置 ──
node scripts/configure.mjs <fsk-key>           # 配置（默认 DeepSeek-V4-Flash）
node scripts/configure.mjs <fsk-key> --env     # 配置（密钥用环境变量）

# ── 查看 ──
node scripts/configure.mjs --show              # 查看当前配置
node scripts/configure.mjs <fsk-key> --list    # 列出可用模型

# ── 切换 ──
node scripts/configure.mjs --switch <model-id> # 切换主模型

# ── 调试 ──
node scripts/configure.mjs <fsk-key> --json    # 输出 JSON 片段（不写文件）
```
