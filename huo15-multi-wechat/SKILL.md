...
description: >-
  ...
  CFBundleIdentifier + 重新签名，绕过微信单实例锁。当用户说"微信多开""开两个微信"
  "多个微信同时登录""multi wechat"等意图时使用。支持创建任意数量的微信副本。
...
微信通过 `CFBundleIdentifier`（`com.tencent.xinWeChat`）实现单实例锁——启动时检查是否已有相同 Bundle ID 的进程在运行，如有则激活旧窗口而非启动新实例。

本方案：复制完整应用 → 修改副本的 `CFBundleIdentifier`（如 `com.tencent.xinWeChat2`）→ 重新签名。系统视为不同应用，绕过单实例锁。
...
脚本自动完成：复制应用 → 修改 Bundle ID → 修改显示名 → 重新签名 → 验证。
...
4. **不要手动改 Info.plist 后不签名** — 修改了 `CFBundleIdentifier` 后必须重新签名，否则无法运行。
5. **副本占用空间** — 每个副本约 300-500MB（完整复制），删除时 `rm -rf` 即可。

## 反模式（禁止）

- ❌ 用轻量启动器（脚本调用 `WeChat.app/Contents/MacOS/WeChat`）做多开——微信单实例锁会直接激活旧窗口，**无效**。
- ❌ 修改 `CFBundleIdentifier` 后不重新签名——应用无法启动。
- ❌ 修改微信二进制文件——可能触发安全检测且无必要。
- ❌ 使用第三方多开工具注入微信——高风险，可能触发风控。

## 核心原则

**复制完整应用 · 改 Bundle ID · 重新签名 · 不注入不改协议。** 这是最安全的多开方式——系统层面视为不同应用，微信层面无法区分。
